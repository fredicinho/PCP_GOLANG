...
Der Einstieg in die Go-Welt fiel sehr leicht. Durch die von Google bereitgestellte, sehr ausführliche Tour of Go konnte eine sehr steile Lernkurve erzielt werden. Dabei wurden die wesentlichen Sprachkonstrukte anhand von einfachen, nachvollziehbaren Beispielen eingeführt. Das Vorgehen war dabei identisch zu anderen Sprachen von Google. \\Das Lösen der Aufgaben bereitete nur bedingt Probleme. Da es sich bei Go um eine Multiparadigmensprache handelt, konnten die Übungen zumeist mit objektorientierten Ansätzen gelöst werden. Jedoch wurden auch explizit Übungen ausgewählt, welche den Einsatz von Funktionen höherer Ordnung nötig machten. \\
Resultierend aus den gemachten Erfahrungen mit der Programmiersprache fällt das Teamfazit positiv aus. Bei einem Projekt mit einer grossen Datenmenge wäre Go die erste Wahl, bei kleineren Projekten würden beide Projektbeteiligten auf bereits besser bekannte Sprachen zurückgreifen. 

\subsection{Fazit Frederico Fischer}

\subsection{Fazit Oliver Werlen}
Go bietet viele, von anderen Sprachen bekannte Konstrukte, direkt von Haus aus und vereinfacht deren Anwendung. Besonders das Schreiben von asynchronem Code fiel mir positiv auf. Durch die Verwendung von Channels kann sehr einfach ein Synchronisierungsmechanismus implementiert werden. Das Modul Konzept von Go macht den Einsatz eines zusätzlichen Dependency Managers wie Gradle oder Maven überflüssig. \\
